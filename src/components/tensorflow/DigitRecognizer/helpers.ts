...
export function doPrediction(
  ...
  // note: argMax gives the index of the highest probability class
  const labels: tf.Tensor<tf.Rank.R1> = testData.labels.argMax(-1);
 ...
 */
export async function showAccuracy(model: tf.Sequential, data: MnistData) {
 ...
 * */
export async function showConfusion(model: tf.Sequential, data: MnistData) {
  const [preds, labels] = doPrediction(model, data);
